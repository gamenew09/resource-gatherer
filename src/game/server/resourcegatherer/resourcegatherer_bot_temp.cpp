...
void Bot_Think( CResourceGatherer_Player *pBot );
...
CBasePlayer *BotPutInServer( bool bFrozen, int iTeam )
{
	...
	CResourceGatherer_Player *pPlayer = ((CResourceGatherer_Player *)CBaseEntity::Instance( pEdict ));
	pPlayer->ClearFlags();
...
void Bot_RunAll( void )
{
	...
	{
		CResourceGatherer_Player *pPlayer = ToRGPlayer( UTIL_PlayerByIndex( i ) );
...
// Output : 	virtual void
//-----------------------------------------------------------------------------
static void RunPlayerMove( CResourceGatherer_Player *fakeclient, const QAngle& viewangles, float forwardmove, float sidemove, float upmove, unsigned short buttons, byte impulse, float frametime )
{
	if ( !fakeclient )
		return;
