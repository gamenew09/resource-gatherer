...
CON_COMMAND( ai_test_los, "Test AI LOS from the player's POV" )
{
	...
	CTraceFilterLOS traceFilter( pPlayer, COLLISION_GROUP_NONE );
	UTIL_TraceLine( pPlayer->EyePosition(), pPlayer->EyePosition() + pPlayer->EyeDirection3D() * MAX_COORD_RANGE, MASK_BLOCKLOS_AND_NPCS, &traceFilter, &tr );
	NDebugOverlay::Line( pPlayer->EyePosition(), tr.endpos, 127, 127, 127, true, 5 );
